...
2. **Account Profiles (Local Persistence)**: 
   - Create accounts with custom names and icons (🧙‍♂️, 🧝‍♀️, 🧑‍🔬, 🦁, 🦊,🦉).
...
3. **Local Ranking System**: 
   - A live ranking badge (👑 #1, 🥈 #2, etc.) is calculated dynamically for each profile based on game wins and gold count.
4. **4 Game Difficulty Levels**:
   - **Easy Level**: Guess from 1 to 50 | 10 Heart Lives | 50s Timer | Reward: 50🪙
   - **Medium Level**: Guess from 1 to 100 | 7 Heart Lives | 80s Timer | Reward: 100🪙
   - **Hard Level**: Guess from 1 to 250 | 5 Heart Lives | 110s Timer | Reward: 200🪙
   - **Expert Level**: Guess from 1 to 500 | 3 Heart Lives | 140s Timer | Reward: 500🪙
5. **Level-Scaled Shop (Hints & Tools)**:
   - **Odd/Even Hint (🔮)**: Checks if the secret number is odd or even.
   - **Range Hint (👁️)**: Narrows the minimum and maximum boundaries.
